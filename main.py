...
def send_whatsapp_alert(title, read_time, topic):
    ...
    # TextMeBot URL split for safety
    tmb_host = "https://api.textmebot.com/send.php"
    url = f"{tmb_host}?recipient={WHATSAPP_PHONE_NUMBER}&apikey={TEXTMEBOT_API_KEY}&text={encoded}"
...
# ==========================================
# 4. GEMINI AI CONTENT GENERATOR (DIVERSIFIED)
# ==========================================
def generate_blog_content():
    print("🤖 [Gemini Engine] Generating trending blog post...")
    
    categories = [
        "Online Earning & Work From Home Ideas",
        "Personal Finance, Saving Money & Investing",
        "Digital Marketing & Social Media Growth Tips",
        "Health, Fitness & Daily Wellness Routine",
        "Latest Tech Gadgets & Smartphone Tips",
        "Travel, Hidden Places & Tourism Guides",
        "Education, Career Growth & Productivity Hacks",
        "Cybersecurity, Online Safety & Privacy"
    ]
    
    selected_category = random.choice(categories)
    print(f"🎯 [DEBUG] Selected Random Category: {selected_category}")
    
    prompt = (
        f"Write a highly engaging, SEO-optimized, comprehensive blog post in {BLOG_LANGUAGE}. "
        f"The post must focus on a popular, trending, and highly searched topic within this specific category: '{selected_category}'. "
        "Do NOT write about Artificial Intelligence (AI) unless the category specifically demands it. "
    ...
    # URL split to prevent markdown auto-linking corruption
    api_base = "[https://generativelanguage.googleapis.com/v1beta/models/](https://generativelanguage.googleapis.com/v1beta/models/)"
    api_model = "gemini-2.5-flash:generateContent"
    url = f"{api_base}{api_model}?key={GEMINI_API_KEY}"
...
# ==========================================
# 5. POLLINATIONS AI IMAGE GENERATOR (BASE64 EMBEDDED)
# ==========================================
def get_featured_image(topic):
    ...
    img_domain = "[https://image.pollinations.ai/prompt/](https://image.pollinations.ai/prompt/)"
    clean_image_url = f"{img_domain}{prompt_encoded}?width=800&height=450&nologo=true"
